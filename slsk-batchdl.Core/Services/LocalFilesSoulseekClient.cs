...
namespace Sldl.Core.Services
{
    public partial class LocalFilesSoulseekClient : ISoulseekClient
    {
        ...
        {
            if (useTags)
                SldlLog.Info($"Reading tags from mock files dir, this may take a while. Use --mock-files-no-read-tags if tags are not needed.");
            else
                SldlLog.Info("Mock files tag reading disabled; using deterministic synthetic track lengths derived from filenames.");
                ...
                .Select((entry, i) =>
                {
                    ...
                    if (Utils.IsMusicFile(path))
                    {
                        if (useTags)
                        {
                            ...
                            catch (Exception ex) { SldlLog.Warn($"Failed to read tags for '{path}': {ex.Message}"); }
                        }
                        else
                        {
